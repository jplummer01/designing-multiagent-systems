...
def test_llm_client_serialization():
    ...
    assert str(loaded_client.client.base_url) == client.api_key  # Note: this might need adjustment based on actual client structure
...
def test_function_tool_serialization_blocked():
    ...
    with pytest.raises(NotImplementedError, match="cannot be serialized for security reasons"):
        tool.dump_component()
...
def test_ai_orchestrator_serialization():
    ...
    # Create components
    model_client = OpenAIChatCompletionClient(model="gpt-4o-mini", api_key="test-key")
    selector_client = OpenAIChatCompletionClient(model="gpt-4o-mini", api_key="selector-key")
...
def test_nested_serialization_integrity():
    ...
    model_client = OpenAIChatCompletionClient(model="gpt-4o-mini", api_key="nested-test")
    ...
    orchestrator = AIOrchestrator(
        ...
        model_client=OpenAIChatCompletionClient(model="gpt-4o-mini", api_key="orchestrator-key"),
        max_iterations=20
    )
    ...
    text_condition = next(c for c in conditions if isinstance(c, TextMentionTermination))
