...
async def main():
    ...
    termination = MaxMessageTermination(max_messages=10) | TextMentionTermination(text="APPROVED")
    ...
    async for item in orchestrator.run_stream(task, verbose=False):
        if isinstance(item, OrchestrationResponse):
            ...
            print(f"   • Agents used: {metadata.get('unique_agents_selected', 0)}/{len(orchestrator.agents)}")
            ...
            if 'selection_history' in metadata and metadata['selection_history']:
                print(f"   • Selection sequence: {' → '.join([sel['agent'] for sel in metadata['selection_history']])}")
        else:
